...
extern const osThreadId accel_tid;
extern const osThreadId temp_tid;
...
void INTTIM_Config(void){
  ...
  NVIC_InitStructure.NVIC_IRQChannelCmd = ENABLE;
	...
	TIM_TimeBaseStructure.TIM_Period = (1000000/20) - 1;
...
void TIM2_IRQHandler(void){
	...
	if(TIM_GetFlagStatus(TIM2, TIM_FLAG_Update)){
		osSignalSet(accel_tid, 0x1);
		TIM_ClearITPendingBit(TIM2, TIM_IT_Update);
	}
}

/**
 * @brief The TIM5 interrupt handler.
 *
 * @note	When TIM5 is high, the ISR signals the temperature thread.
 */
void TIM5_IRQHandler(void){
	//TIM_GetFlagStatus returns status flag, where 0 == Reset, Set == !Reset == 1
	if(TIM_GetFlagStatus(TIM5, TIM_FLAG_Update)){
		osSignalSet(temp_tid, 0x1);
